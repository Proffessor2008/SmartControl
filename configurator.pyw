...
with open(Path('files/config.json').resolve(), 'r', 'utf-8') as data:
    config = json.load(data)
    data.close()

with open(Path('files/com_list.txt').resolve(), 'r', 'utf-8') as data:
    com_list = data.read()
    data.close()
...
def add():
    ...
    ln = ln_entry.get()
    with open(Path('files/kw.json').resolve(), 'r', 'utf-8') as f:
        f_kw = json.load(f)
        for i in kw:
            try:
    ...
    with open(Path('files/kw.json').resolve(), 'w', 'utf-8') as f:
        json.dump(f_kw, f, ensure_ascii=False)
        f.close()

    with open(Path('files/we.json').resolve(), 'r', 'utf-8') as f:
        f_we = json.load(f)
        f_we['open_'][ln] = 0
        f.close()

    with open(Path('files/we.json').resolve(), 'w', 'utf-8') as f:
        json.dump(f_we, f, ensure_ascii=False)
        f.close()


label_com_list = customtkinter.CTkLabel(master=app, text='Список команд', bg_color=colorback,
...
label_add_com = customtkinter.CTkLabel(master=app, text='Добавить сайт/файл', bg_color=colorback,
                                       font=('TkHeadingFont', 15.1))
...
button_1 = customtkinter.CTkButton(master=app, text='Сохранить', width=230, fg_color=color1, hover_color=color2,
                                   font=('TkHeadingFont', 15), command=add)
